...
import numpy as np
import sys
from DDQNmodel import DuelingDQN, Buffer
...
class ModelTrain():
    ...
    def __init__(self, width, height, bomb_no, render_flag):
        ...
        self.current_model = DuelingDQN(self.box_count, self.box_count)
        self.target_model = DuelingDQN(self.box_count, self.box_count)
        ...
        self.tau = 5e-5
        self.log = open("./training_log_DDQN.txt", 'w')
        if self.render_flag:
